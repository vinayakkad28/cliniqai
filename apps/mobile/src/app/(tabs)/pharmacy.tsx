    ...
    <TouchableOpacity style={styles.card}>
      <View style={styles.cardTop}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.medGeneric}>{med.generic} &middot; {med.category}</Text>
        </View>
      ...
      <View style={styles.cardBottom}>
        <View style={styles.cardStat}>
          ...
          <Text style={[styles.statValue, med.stock < LOW_THRESHOLD && { color: "#dc2626" }]}>
            {med.stock} {med.unit}
          </Text>
        </View>
        <View style={styles.cardStat}>
          <Text style={styles.statLabel}>Batch Expiry</Text>
          <Text style={[styles.statValue, isExpiringSoon(med.batchExpiry) && { color: "#d97706" }]}>
            {med.batchExpiry}
          </Text>
        </View>
        <View style={styles.cardStat}>
          <Text style={styles.statLabel}>MRP</Text>
          <Text style={styles.statValue}>₹{med.mrp}</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
...
export default function PharmacyScreen() {
  ...
  const filtered = INVENTORY.filter((m) => {
    ...
    if (tab === "low") return m.stock > 0 && m.stock < LOW_THRESHOLD;
    if (tab === "out") return m.stock === 0;
    if (tab === "expiring") return isExpiringSoon(m.batchExpiry);
    return true;
  });
...
const styles = StyleSheet.create({
  ...
  medGeneric: { fontSize: 12, color: "#6b7280", marginTop: 2 },
  ...
  statValue: { fontSize: 13, color: "#374151", fontWeight: "600", marginTop: 1 },
